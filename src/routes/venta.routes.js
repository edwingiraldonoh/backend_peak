...
router.post('/', async (req, res) => {
    const {total_venta, mesero_encargado, comision} = req.body;

    if (!total_venta || !comision || !mesero_encargado) {
        return res.status(400).json({error: 'Datos requeridos'});
    }

    try {
        const [result] = await pool.query(
            'INSERT INTO venta (total_venta, mesero_encargado, comision) VALUES (?, ?, ?)',
            [nombre, descripcion, precio]
        );
        res.status(201).json({ id: result.insertId, total_venta, mesero_encargado, comision });
    } catch (error) {
...
router.put('/:id_venta', async (req, res) => {
    ...
    try {
        const [result] = await pool.query(
            'UPDATE venta SET total_venta = ?, mesero_encargado = ?, comision = ? WHERE id_venta = ?',
            [total_venta, mesero_encargado, comision, req.params.id]
        );
