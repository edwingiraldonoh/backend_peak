...
router.get('/:id_notificacion', async (req, res) => {
    try {
...
router.post('/', async (req, res) => {
    const {mensaje_notificacion, estado_notificacion, destinatario} = req.body;

    if (!mensaje_notificacion || !estado_notificacion || !destinatario) {
        return res.status(400).json({error: 'Datos requeridos obligatoriamente'});
    }

    try {
        const [result] = await pool.query(
            'INSERT INTO notificacion (mensaje_notificacion, estado_notificacion, destinatario) VALUES (?, ?, ?)',
            [mensaje_notificacion, estado_notificacion, destinatario]
        );
        res.status(201).json({ id: result.insertId, mensaje_notificacion, estado_notificacion, destinatario });
    } catch (error) {
...
router.put('/:id_notificacion', async (req, res) => {
    ...
    try {
        const [result] = await pool.query(
            'UPDATE notificacion SET mensaje_notificacion = ?, estado_notificacion = ?, destinatario = ? WHERE id_notificacion = ?',
            [mensaje_notificacion, estado_notificacion, destinatario, req.params.id]
        );
