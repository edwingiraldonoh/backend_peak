...
router.post('/api/register', async (req, res) => {
    const {nombre_usuario, apellido_usuario, contraseña, correo_electronico, telefono} = req.body;
    ...
    try {
        const [result] = await pool.query(
            'INSERT INTO usuarios (nombre_usuario, apellido_usuario, estado, rol, contraseña, correo_electronico, telefono) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [nombre_usuario, apellido_usuario, contraseña, correo_electronico, telefono]
        );
        res.status(201).json({ id: result.insertId, nombre_usuario, apellido_usuario, contraseña, correo_electronico, telefono });
    } catch (error) {
...
router.put('/:id_usuario', async (req, res) => {
    ...
    try {
        const [result] = await pool.query(
            'UPDATE usuarios SET nombre_usuario = ?, apellido_usuario = ?, contraseña = ?, correo_electonico = ?, telefono = ? WHERE id_usuario = ?',
            [nombre_usuario, apellido_usuario ,contraseña, correo_electronico, telefono, req.params.id]
        );
